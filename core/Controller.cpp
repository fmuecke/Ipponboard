...
//=========================================================
{
	...
	for (int round = 0; round < m_mode.nRounds; ++round)
	{
		...
		{
			QString weight = m_mode.weightsAreDoubled ? actualWeights[fightNo / 2] : actualWeights[fightNo];
...
//=========================================================
{
	...
	switch (point)
	{
	case Point::Yuko: value = score.Yuko(); break;

	case Point::Wazaari: value = /*score.IsAwaseteIppon() ? 0 : */ score.Wazaari(); break;
	...
	case Point::Shido: value = score.Shido(); break;

	case Point::Hansokumake: value = score.Hansokumake() ? 1 : 0; break;
...
//=========================================================
{
	if (doRevoke)
	{
		switch (action)
		{
		case eAction_Yuko: m_pSM->process_event(IpponboardSM_::RevokeYuko(whos)); break;

		case eAction_Wazaari: m_pSM->process_event(IpponboardSM_::RevokeWazaari(whos)); break;

		case eAction_Ippon: m_pSM->process_event(IpponboardSM_::RevokeIppon(whos)); break;

		case eAction_Shido:
		case eAction_Hansokumake: m_pSM->process_event(IpponboardSM_::RevokeShidoHM(whos)); break;

		case eAction_ResetOsaeKomi: reset_timer_value(eTimer_Hold); break;

		case eAction_ResetMainTimer: reset_timer_value(eTimer_Main); break;
	...
	else
	{
		...
		case eAction_Yuko: m_pSM->process_event(IpponboardSM_::Yuko(whos)); break;

		case eAction_Wazaari: m_pSM->process_event(IpponboardSM_::Wazaari(whos)); break;

		case eAction_Ippon: m_pSM->process_event(IpponboardSM_::Ippon(whos)); break;

		case eAction_Shido: m_pSM->process_event(IpponboardSM_::Shido(whos)); break;

		case eAction_Hansokumake: m_pSM->process_event(IpponboardSM_::Hansokumake(whos)); break;

		case eAction_SetOsaekomi: m_Tori = whos; break;

		case eAction_ResetAll: m_pSM->process_event(IpponboardSM_::Reset()); break;
	...
	//
	if (current_fight().IsGoldenScore() && action != eAction_Hajime_Mate && action != eAction_Shido &&
	    eState_TimerStopped != EState(m_pSM->current_state()[0]))
	{
		...
		if (eState_Holding != EState(m_pSM->current_state()[0]))
		{
			...
			if (ruleSet->CompareScore(current_fight()) != 0) { m_pSM->process_event(IpponboardSM_::Hajime_Mate()); }
		}
	}
...
//=========================================================
{
	...
	case eState_Holding: winner = m_Tori; break;
...
//=========================================================
{
	...
	else if (eTimer_Hold == timer)
	{
		...
		if (!m_pTimerHold->isActive()) m_Tori = FighterEnum::Nobody;
	}
}
...
//=========================================================
{
	...
	QString name = m_Tournament[m_currentRound]->at(m_currentFight).GetFighter(who).name;

	// shorten name
	const int pos = name.indexOf(' ');

	if (pos != -1) { name = name.left(1) + ". " + name.mid(pos, name.length() - pos); }
...
//=========================================================
{
	...
	if (pos != -1) { name = name.mid(pos + 1, name.length() - pos); }
...
//=========================================================
{
	...
	return m_Tournament[m_currentRound]->at(m_currentFight).GetFighter(who).club;
}
...
//=========================================================
{
	return m_Tournament[m_currentRound]->at(m_currentFight).weight;
}
...
//=========================================================
{
	...
	for (size_t round(0); round < m_Tournament.size(); ++round)
	{
		...
		{
			if (m_Tournament[round]->at(fight).is_saved) { score += m_Tournament[round]->at(fight).HasWon(who); }
		}
	}
...
//=========================================================
{
	if (eState_TimerStopped == m_State || eState_SonoMama == m_State)
	{
		if (eTimer_Main == timer)
		{
			...
			if (newTime.isValid()) { *m_pTimeMain = newTime; }
		}
		else if (eTimer_Hold == timer) { m_pTimeHold->setHMS(0, 0, value.toInt()); }
...
void Controller::OverrideRoundTimeOfFightMode(int fightTimeSecs) { m_mode.fightTimeInSeconds = fightTimeSecs; }
...
//=========================================================
{
	...
	else { *m_pTimeMain = QTime(0, 0, 0, 0).addSecs(current_fight().GetRemainingTime()); }
...
void Controller::SetRules(std::shared_ptr<AbstractRules> rules)
{
	...
	{
		for (auto& fight : *pRound) { fight.rules = rules; }
	}
}
...
//=========================================================
{
	...
	// TODO: remove maybe
	if (eOption_AllSubscoresCount == option) { m_rules->SetCountSubscores(isSet); }
}
...
//=========================================================
{
	if (m_gongFile.isEmpty()) { return; }

	if (!m_gongEffect) { m_gongEffect = std::make_unique<QSoundEffect>(); }
...
//=========================================================
{
	...
	{
		if (timer == eTimer_Main) { update_main_time(); }
		else if (timer == eTimer_Hold) { update_hold_time(); }
	}
}
//=========================================================
...
//=========================================================
{
	if (eTimer_Main == t) { m_pTimerMain->start(1000); }
	else { m_pTimerHold->start(1000); }
}
...
//=========================================================
{
	...
	if (eTimer_Main == t) { m_pTimerMain->stop(); }
}
...
//=========================================================
{
	...
	return m_Tournament[m_currentRound]->at(m_currentFight).GetScore(who);
}
...
//=========================================================
{
	if (eTimer_Hold == t) { return -m_pTimeHold->secsTo(QTime(0, 0, 0, 0)); }
	else { return m_pTimeMain->secsTo(QTime(0, 0, 0, 0)); }
}
...
//=========================================================
{
	...
	else { SetCurrentFight(currentFight + 1); }
}
...
//=========================================================
{
	...
	if (ippon1 > 0) { fight.GetScore1().Add(Ippon); }
	...
	if (hansokumake1 > 0) { fight.GetScore1().Add(Hansokumake); }
	...
	if (ippon2 > 0) { fight.GetScore2().Add(Ippon); }
	...
	if (hansokumake2 > 0) { fight.GetScore1().Add(Hansokumake); }
...
//=========================================================
Ipponboard::Fight const& Controller::GetFight(unsigned int round_index, unsigned int fight_index) const
//=========================================================
{
...
//=========================================================
{
	m_Tournament[m_currentRound]->at(m_currentFight).fighters[whos].name = name;
...
//=========================================================
{
	...
	for (auto const& pModel : m_TournamentModels) { pModel->SetDataChanged(); }
}
...
//=========================================================
{
	if ((size_t)which < m_TournamentModels.size() && which > 0) return m_TournamentModels[which];
...
//=========================================================
{
	if (m_rules->IsOption_OpenEndGoldenScore() && is_golden_score()) { *m_pTimeMain = m_pTimeMain->addSecs(1); }
	else
	{
		*m_pTimeMain = m_pTimeMain->addSecs(-1);
		...
		if (secsTo < 0 || *m_pTimeMain > m_roundTime) m_pTimeMain->setHMS(0, 0, 0, 0);
...
//=========================================================
{
	if (eState_Holding != m_State) return;
	...
	*m_pTimeHold = m_pTimeHold->addSecs(1);
	                 ...
	                 m_rules->GetOsaekomiValue(Score::Point::Ippon) == secs))
	{
		...
		if (eState_TimerStopped == m_State) Gong();
	}
...
//=========================================================
{
	for (auto const& pView : m_views) { pView->UpdateView(); }

	for (auto const& pView : m_goldenScoreViews) { pView->UpdateGoldenScoreView(); }
}
