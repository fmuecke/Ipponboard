...
int Ipponboard::AbstractRules::CompareScore(const Fight& f) const
{
	...
	if (lhs.Value(Point::Hansokumake) < rhs.Value(Point::Hansokumake)) { return -1; }

	if ((lhs.Value(Point::Hansokumake) > rhs.Value(Point::Hansokumake))) { return 1; }

	if (lhs.Value(Point::Ippon) > rhs.Value(Point::Ippon)) { return -1; }

	if (lhs.Value(Point::Ippon) < rhs.Value(Point::Ippon)) { return 1; }

	if (lhs.Value(Point::Wazaari) > rhs.Value(Point::Wazaari)) { return -1; }

	if (lhs.Value(Point::Wazaari) < rhs.Value(Point::Wazaari)) { return 1; }

	if (lhs.Value(Point::Yuko) > rhs.Value(Point::Yuko)) { return -1; }
	...
	{
		if (lhs.Value(Point::Shido) < rhs.Value(Point::Shido)) { return -1; }
