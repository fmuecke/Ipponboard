...
::qdirect2dd.dll qminimald.dll qoffscreend.dll"
...
if "%RELEASE%"=="1" (
	for %%F in (%qtFiles%) do (copy /Y "%QT5DIR%\bin\%%F" "%DEST_DIR%" >nul || exit /b 1)
	for %%F in (%qtPluginFiles%) do (copy /Y "%QT5DIR%\plugins\platforms\%%F" "%DEST_DIR%\plugins\platforms" >nul || exit /b 1)
	for %%F in (%vcRedistFiles%) do (copy /Y "%VCToolsRedistDir%x86\Microsoft.VC143.CRT\%%F" "%DEST_DIR%">nul || exit /b 1)
) else (
	for %%F in (%qtDebugFiles%) do (copy /Y "%QT5DIR%\bin\%%F" "%DEST_DIR%" >nul || exit /b 1) 
	for %%F in (%qtDebugPluginFiles%) do (copy /Y "%QT5DIR%\plugins\platforms\%%F" "%DEST_DIR%\plugins\platforms" >nul || exit /b 1)
