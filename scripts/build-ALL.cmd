...
call %~dp0create-versioninfo.cmd %IPPONBOARD_ROOT_DIR%\base || exit /b %errorlevel%

echo Creating makefiles...
cmake -S "%IPPONBOARD_ROOT_DIR%" -B "%BUILD_DIR%" -G "Visual Studio 17 2022" -A Win32 || exit /b %errorlevel%

echo Building tests...
cmake --build "%IPPONBOARD_ROOT_DIR%\_build\build-Ipponboard" --config %CONFIG% %CLEAN% --target IpponboardTest || exit /b %errorlevel%
...
echo Building Ipponboard...
cmake --build "%IPPONBOARD_ROOT_DIR%\_build\build-Ipponboard" --config %CONFIG% %CLEAN% || exit /b %errorlevel%