...
{
static bool ReadItems(QString const& fileName, QString const& separator, std::vector<QStringList>& readData,
                      QString& errorMsg)
{
	errorMsg.clear();
	...
	for (QString line = in.readLine(); !line.isNull(); ++lineNo)
	{
		...
		if (itemsPerLine == 0) { itemsPerLine = splittedLine.length(); }
