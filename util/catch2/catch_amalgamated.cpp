                           ...
                           double (*estimator)(double const*, double const*))
{
    ...
    double prob_n =
        static_cast<double>(std::count_if(
            resample.begin(), resample.end(), [point](double x) { return x < point; })) /
...
IEventListenerPtr prepareReporters(Config const* config)
{
        ...
        config->getProcessedReporterSpecs().size() == 1)
    {
        ...
        return createReporter(
            spec.name,
            ReporterConfig(
                config, makeStream(spec.outputFilename), spec.colourMode, spec.customOptions));
    }
    ...
    {
        multi->addReporter(createReporter(reporterSpec.name,
    ...
    if (!guardFilePath.empty())
    {
        ...
        if (!file)
        {
            ...
            CATCH_RUNTIME_ERROR("Could not open the exit guard file '"
                                << guardFilePath << "' because '" << errMsg << "' (" << err << ')');
        }
      ...
      lineInfo(_lineInfo)
{
    ...
    CATCH_ENFORCE(!inTag,
                  "Found an unclosed tag while registering test case '" << _nameAndTags.name
                                                                        << "' at " << _lineInfo);
                                          ...
                                          Detail::TokenStream tokens) const
{
    ...
    while (result.value().remainingTokens())
    {
        ...
        for (auto& parseInfo : parseInfos)
        {
            ...
            {
                result =
                    parseInfo.parser->parse(exeName, CATCH_MOVE(result).value().remainingTokens());
                if (!result)
                    return result;
...
class Win32ColourImpl final : public ColourImpl
{
  ...
  private:
    ...
    defined(__FreeBSD__) || defined(CATCH_PLATFORM_QNX)
...
static constexpr SignalDefs signalDefs[] = { { SIGINT, "SIGINT - Terminal interrupt signal" },
...
std::map<std::string, IReporterFactoryPtr, Detail::CaseInsensitiveLess> const& ReporterRegistry::
    getFactories() const
{
...
Totals RunContext::runTest(TestCaseHandle const& testCase)
{
    ...
    do
    {
        ...
        auto statsForOneRun = TestCaseStats(
            testInfo, singleRunTotals, CATCH_MOVE(oneRunCout), CATCH_MOVE(oneRunCerr), aborting());
    ...
    m_reporter->testCaseEnded(TestCaseStats(
        testInfo, deltaTotals, CATCH_MOVE(redirectedCout), CATCH_MOVE(redirectedCerr), aborting()));
...
void RunContext::handleMessage(AssertionInfo const& info, ResultWas::OfType resultType,
                               std::string&& message, AssertionReaction& reaction)
{
    Detail::g_lastKnownLineInfo = info.lineInfo;
...
AssertionInfo RunContext::makeDummyAssertionInfo()
{
    const bool testCaseJustStarted =
        Detail::g_lastKnownLineInfo == m_activeTestCase->getTestCaseInfo().lineInfo;

    return AssertionInfo{ testCaseJustStarted ? "TEST_CASE"_sr : StringRef(),
                          Detail::g_lastKnownLineInfo,
                          testCaseJustStarted ? StringRef()
...
void hexEscapeChar(std::ostream& os, unsigned char c)
{
    ...
    os << "\\x"_sr << std::uppercase << std::hex << std::setfill('0') << std::setw(2)
       << static_cast<int>(c);
    os.flags(f);
}
...
std::string WithinUlpsMatcher::describe() const
{
    ...
    else
    {
        ...
        write(ret,
              step(static_cast<float>(m_target), -std::numeric_limits<float>::infinity(), m_ulps));
        ret << ", ";
        write(ret,
              step(static_cast<float>(m_target), std::numeric_limits<float>::infinity(), m_ulps));
    }
...
{
    defaultListTests(
        m_stream, m_colour.get(), tests, m_config->hasTestFilters(), m_config->verbosity());
}
    ...
    if (!tags.empty())
    {
        const auto maxTagCount =
            std::max_element(tags.begin(),
                             tags.end(),
                             [](auto const& lhs, auto const& rhs) { return lhs.count < rhs.count; })
                ->count;
