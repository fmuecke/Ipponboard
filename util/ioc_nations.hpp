...
static Nation nations[204 + 1] = { { 0, "Code", "NATION" },
	                               { 1, "AFG", "Afghanistan" },
	                               { 2, "ALB", "Albania" },
	                               { 3, "ALG", "Algeria" },
	                               { 4, "AND", "Andorra" },
...
{
	auto i = std::find_if(&nations[0], &nations[0] + ArrayLength(nations), [=](Nation n) { return code == n.code; });
	if (i == &nations[0] + ArrayLength(nations)) return nations[0].name;
...
{
	auto i = std::find_if(&nations[0], &nations[0] + ArrayLength(nations), [=](Nation n) { return name == n.name; });
	if (i == &nations[0] + ArrayLength(nations)) return nations[0].code;
...
{
	QStringList list;
	for (Nation const& n : nations) { list.push_back(n.name); }
	return list;
}
}; // namespace IocNations
