...
TEST_CASE("ToJson serializes provided data")
{
	...
	CHECK(firstFight["FirstFighter"].toObject()["Name"].toString() == data.rounds.front().front().GetFighter(FighterEnum::First).name);
	CHECK(firstFight["SecondFighter"].toObject()["Wazaari"].toInt() == data.rounds.front().front().GetScore2().Value(Score::Point::Wazaari));
}
...
TEST_CASE("CreateFromJson reports version mismatch")
{
	...
	const auto refusal = TournamentSerialization::CreateFromJson(
		mismatchingDoc,
		QString::fromLatin1(TournamentSaveFileVersion),
		false,
		parsed);
	REQUIRE(refusal == 1);
