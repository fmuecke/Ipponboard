...
class Gamepad
{
  public:
	...
	inline UnsignedPair GetPollingFreq() const { return std::make_pair(m_caps.wPeriodMin, m_caps.wPeriodMax); }
	...
	{
		if (0 == (m_caps.wCaps & JOYCAPS_HASPOV)) return ePovType_no_pov;
	...
	bool WasPressed(EButton b) const
	{
		...
		if (b > eButton32)
		{
			if (button_code[b] == m_data.dwPOV && m_data.dwPOV != m_lastData.dwPOV) { return true; }

			return false;
		}

		return (m_lastData.dwButtons & button_code[b]) == 0 && (m_data.dwButtons & button_code[b]) > 0;
	}
	...
	{
		const int lastX = m_invertedAxes.test(eAxis_X) ? eMax - m_lastData.dwXpos : m_lastData.dwXpos;
		const int lastY = m_invertedAxes.test(eAxis_Y) ? eMax - m_lastData.dwYpos : m_lastData.dwYpos;
		...
		if (-1.0f == lastAlpha || -1.0f == curAlpha || fabs(lastAlpha - curAlpha) > eTolerance) { changed = true; }

		if (changed && IsInSection(curAlpha, min, max)) { return true; }
	...
	{
		const int lastR = m_invertedAxes.test(eAxis_R) ? eMax - m_lastData.dwRpos : m_lastData.dwRpos;
		const int lastZ = m_invertedAxes.test(eAxis_Z) ? eMax - m_lastData.dwZpos : m_lastData.dwZpos;
	...
	bool IsInSection(const float alpha, const float min, const float max) const
	{
		...
		if (min > max)
		{
			if (min <= alpha && alpha <= 360.0f) { ret = true; }
			else if (0 < alpha && alpha <= max) { ret = true; }
		}
		else
		{
			if (min <= alpha && max >= alpha) { ret = true; }
		}
	...
	template <class T> T GetAngle(int x, int y, float factor = 0.0f, int max = 0) const
	{
		...
		if (h * factor < (float)max) { return -1; }
	...
	//FIXME: use double
	float GetAngleXY() const { return GetAngle<float>(GetXPos() - eMid, GetYPos() - eMid, 1.2f, eMid); }
	float GetAngleRZ() const { return GetAngle<float>(GetRPos() - eMid, GetZPos() - eMid, 1.2f, eMid); }

	int PressedCount() const { return m_data.dwButtonNumber; }

	unsigned GetXPos() const
	{
		if (m_invertedAxes.test(eAxis_X)) return 65535 - m_data.dwXpos;
	...
	{
		if (m_invertedAxes.test(eAxis_Y)) return 65535 - m_data.dwYpos;
	...
	{
		if (m_invertedAxes.test(eAxis_Z)) return 65535 - m_data.dwZpos;
	...
	{
		if (m_invertedAxes.test(eAxis_R)) return 65535 - m_data.dwRpos;

		return m_data.dwRpos;
	}
	unsigned GetUPos() const
	{
		if (m_invertedAxes.test(eAxis_U)) return 65535 - m_data.dwUpos;
	...
	{
		if (m_invertedAxes.test(eAxis_V)) return 65535 - m_data.dwVpos;
	...
	unsigned GetThreshold() const
	{
		...
		if (eState_ok != ::joyGetThreshold(m_currentId, &t)) return static_cast<unsigned>(-1);
	...
	bool SetThreshold(unsigned threshold) const { return eState_ok == ::joySetThreshold(m_currentId, threshold); }
