...
void SetTranslation(QApplication& app, QTranslator& translator, QString const& langStr)
{
	...
	if (langStr == QString("de") || langStr == QString("nl"))
	{
		...
		if (translator.load(langStr, langPath)) { app.installTranslator(&translator); }
		else { LangNotFound(langStr); }
	}
}
...
int ShowSplashScreen()
{
	...
	auto t3 =
	    QCoreApplication::tr("Therefore, Ipponboard is not just a simple Judo display program, but developed with "
	...
	auto t4 = QCoreApplication::tr("Ipponboard is not only clearly readable but also revolutionary easy and intuitive "
	                               "to use. Therefore, it is appreciated by clubs and organizations around the world "
	                               "and has been used for many years at major championships.");
	...
	auto text =
	    QString("<html><body><p><big><span style=\"color:#336699;font-weight:bold\">Ipponboard</span> - %1</big></p>"
	...
	splashData.info = QCoreApplication::applicationName() + " v" + QCoreApplication::applicationVersion() + "\n" +
	...
	if (QDialog::Rejected == dlgResult) { return 0; }
...
int main(int argc, char* argv[])
{
	...
	if (!validModes.contains(mode)) { parser.showHelp(ERROR_INVALID_PARAMETER); }
	...
	if (settings.contains(str_tag_Language)) { langStr = settings.value(str_tag_Language).toString(); }
	...
	if (onlineVersion.state == OnlineVersionChecker::State::NewerAvailable)
	{
		// format message
		QString changes =
		    QString(": <br><br><tt>%1</tt><br>")
		        .arg(QCoreApplication::tr("en") == "de" ? onlineVersion.changes_de : onlineVersion.changes_en);
		changes.replace("\n", "<br>");
		...
		msg +=
		    QString("<p>%1</p>").arg(QCoreApplication::tr("Do you want to download it or visit the project homepage?"));
	...
	if (mode == "ask") { dlgResult = ShowSplashScreen(); }
	else if (mode == "single") { dlgResult = QDialog::Accepted; }
	else if (mode == "team") { dlgResult = QDialog::Accepted + 1; }
	if (dlgResult == 0) { return NO_ERROR; }

	std::unique_ptr<MainWindowBase> pMainWnd{ nullptr };

	if (dlgResult == QDialog::Accepted + 1) { pMainWnd = std::make_unique<MainWindowTeam>(); }
	else { pMainWnd = std::make_unique<MainWindow>(); }
