...
//=========================================================
{
	...
	QColor bgColor1 = GetColor_(eBlueBg);
	QColor fgColor1 = GetColor_(eBlueFg);
	QColor bgColor2 = GetColor_(eWhiteBg);
	QColor fgColor2 = GetColor_(eWhiteFg);
	...
	case eState_TimerRunning:
		{
			...
			{
				UpdateHoldClock_( Qt::darkGray, m_TextBgColorWhite, eTimer_Hold_White ); // Note: order is important!
				UpdateHoldClock_( Qt::lightGray, m_TextBgColorBlue, eTimer_Hold_Blue );
			}
			else if ( eFighter_White == holder )
			{
				UpdateHoldClock_( Qt::lightGray, m_TextBgColorBlue, eTimer_Hold_Blue ); // Note: order is important!
				UpdateHoldClock_( Qt::darkGray, m_TextBgColorWhite, eTimer_Hold_White );
			}
			else
			{
				UpdateHoldClock_( Qt::gray, Qt::black, eTimer_Holdi );
			}
		}
		break;
	...
	case eState_Holding:
		{
			...
			{
				UpdateHoldClock_( Qt::darkGray, m_TextBgColorWhite, eTimer_Hold_White ); // Note: order is important!
				UpdateHoldClock_( m_TextColorBlue, m_TextBgColorBlue, eTimer_Hold_Blue );
			}
			else if ( eFighter_White == holder )
			{
				UpdateHoldClock_( Qt::lightGray, m_TextBgColorBlue, eTimer_Hold_Blue ); // Note: order is important!
				UpdateHoldClock_( m_TextColorWhite, m_TextBgColorWhite, eTimer_Hold_White );
			}
			//else
			//{
			//	assert(!"wrong hold state");
			//	UpdateHoldClock_( Qt::gray, Qt::black, eTimer_Holdi );
			//}
			#ifdef HORIZONTAL_VIEW
...
#else
	ui->text_hold_clock_blue->setAlignment(Qt::AlignLeft);
	ui->text_hold_clock_white->setAlignment(Qt::AlignRight);
#endif
...
//=========================================================
{
	ui->text_hold_clock_blue->SetColor(Qt::gray, GetColor_(eBlueBg));
	ui->text_hold_clock_white->SetColor(Qt::gray, GetColor_(eWhiteBg));
...
//=========================================================
{
	...
	if (!child)
	{
		...
		else if( child == ui->text_hold_clock_white ) // Note: we are on primaryView, so sides are switched!
		{
			if( doRevoke )  // right click!
			{
				QMenu menu;
				...
				item = menu.addAction(tr("Set Value"));
				connect( item, SIGNAL(triggered()), this,
						 SLOT(setOsaekomiTimerValueBlue_()) );

				item = menu.addAction(tr("Reset"));
				connect( item, SIGNAL(triggered()), this,
						 SLOT(resetOsaekomiTimerValueBlue_()) );

				menu.exec(QCursor::pos());

				return;
			}
			whos = eFighter_White; // will be switched later
			action = eAction_OsaeKomi_Toketa;
		}
		else if( child == ui->text_hold_clock_blue ) // Note: we are on primaryView, so sides are switched!
		{
			if( doRevoke )  // right click!
			{
				QMenu menu;
				...
				item = menu.addAction(tr("Set Value"));
				connect( item, SIGNAL(triggered()), this,
						 SLOT(setOsaekomiTimerValueWhite_()) );

				item = menu.addAction(tr("Reset"));
				connect( item, SIGNAL(triggered()), this,
						 SLOT(resetOsaekomiTimerValueWhite_()) );
...
//=========================================================
void View::setOsaekomiTimerValueBlue_()
//=========================================================
{
	bool ok(false);
	const int seconds = QInputDialog::getInt(
		...
		&ok);
	if( ok )
		m_pController->SetTimerValue( eTimer_Hold_Blue, QString::number(seconds));
}
//=========================================================
void View::setOsaekomiTimerValueWhite_()
//=========================================================
{
	bool ok(false);
	const int seconds = QInputDialog::getInt(
		...
		&ok);
	if( ok )
		m_pController->SetTimerValue( eTimer_Hold_White, QString::number(seconds));
}
...
//=========================================================
{
	QString valueBlue(m_pController->GetTimeText(eTimer_Hold_Blue));
	QString valueWhite(m_pController->GetTimeText(eTimer_Hold_White));

	const EFighter tori = (t==eTimer_Hold_Blue)? eFighter_Blue : eFighter_White;
	ScaledText* pClockText = (tori == GVF_(eFighter_Blue)) ?
							 ui->text_hold_clock_blue :
							 ui->text_hold_clock_white;

	ScaledText* pClockTextUke = (tori == GVF_(eFighter_Blue)) ?
							 ui->text_hold_clock_white :
							 ui->text_hold_clock_blue;
	...
	case eTimer_Hold_Blue:
		pClockText->SetText(valueBlue);
		if( valueBlue == "00" /*&& eState_Holding != m_pController->GetCurrentState()*/ )
		{
			if( IsSecondary_() )
				ui->layout_osaekomi->setStretchFactor(pClockText, 0);
		}
		else
		{
			ui->layout_osaekomi->setStretchFactor(pClockText, 5);
		}
		break;

	case eTimer_Hold_White:
		pClockText->SetText(valueWhite);
		if( valueWhite == "00" /*&& eState_Holding != m_pController->GetCurrentState()*/ )
		{
			if( IsSecondary_() )
				ui->layout_osaekomi->setStretchFactor(pClockText, 0);
		}
	...
	case eTimer_Holdi:
	default:
		pClockTextUke->SetColor( text, bg );
		pClockText->SetText(valueBlue);
		pClockTextUke->SetText(valueWhite);
		ui->image_sand_clock->SetBgColor( Qt::black );
		if( IsSecondary_() )
		{
			ui->layout_osaekomi->setStretchFactor(pClockText, valueBlue=="00" ? 0 : 5);
			ui->layout_osaekomi->setStretchFactor(pClockTextUke, valueWhite=="00" ? 0 : 5);
			if( valueBlue=="00" && valueWhite=="00" )
...
//		pClockText->SetColor( Qt::gray, Qt::black );
//		ui->image_sand_clock->SetBgColor( Qt::black );
//		if( IsSecondary_() )
//		{
//			pClockText->SetText( "" );
...
//			ui->layout_info->setStretchFactor(ui->layout_osaekomi, 0);
//			//ui->layout_osaekomi->setStretchFactor(pClockText, 0);
//			//ui->layout_osaekomi->setStretchFactor(pClockTextUke, 0);
//		#else
...
//=========================================================
{
	ui->text_score_team_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->dummy_blue->SetBgColor(GetColor_(eBlueBg));
	ui->image_hansokumake_blue->SetBgColor(GetColor_(eBlueBg));
	ui->image_shido3_blue->SetBgColor(GetColor_(eBlueBg));
	ui->image_shido2_blue->SetBgColor(GetColor_(eBlueBg));
	ui->image_shido1_blue->SetBgColor(GetColor_(eBlueBg));
	ui->text_yuko_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_wazaari_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_ippon_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_ippon_desc2->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_wazaari_desc2->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_yuko_desc2->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_lastname_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));
	ui->text_firstname_blue->SetColor(GetColor_(eBlueFg), GetColor_(eBlueBg));

	ui->text_score_team_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->dummy_white->SetBgColor(GetColor_(eWhiteBg));
	ui->image_hansokumake_white->SetBgColor(GetColor_(eWhiteBg));
	ui->image_shido3_white->SetBgColor(GetColor_(eWhiteBg));
	ui->image_shido2_white->SetBgColor(GetColor_(eWhiteBg));
	ui->image_shido1_white->SetBgColor(GetColor_(eWhiteBg));
	ui->text_yuko_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_wazaari_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_ippon_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_ippon_desc1->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_wazaari_desc1->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_yuko_desc1->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_lastname_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
	ui->text_firstname_white->SetColor(GetColor_(eWhiteFg), GetColor_(eWhiteBg));
}
