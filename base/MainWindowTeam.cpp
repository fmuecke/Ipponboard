...
{
    m_pUi->button_golden_score->setEnabled(
        m_pController->GetRules()->IsOption_OpenEndGoldenScore());
...
void MainWindowTeam::WriteScoreToHtml_()
{
    ...
    {
        QMessageBox::critical(
            this, tr("File open error"), tr("File could not be opened: ") + file.fileName());
        return;
    }
    ...
    auto totalScore =
        std::make_pair(score1st.first + score2nd.first, score1st.second + score2nd.second);
...
int MainWindowTeam::LoadTournamentFromJson_(QJsonDocument& doc, bool loadWithIncompatibleVersion)
{
    ...
    MainWindowBase::update_text_color_first(QColor::fromRgba(saveData.firstFg),
                                            QColor::fromRgba(saveData.firstBg));
    MainWindowBase::update_text_color_second(QColor::fromRgba(saveData.secondFg),
                                             QColor::fromRgba(saveData.secondBg));
...
void MainWindowTeam::on_actionManageModes_triggered()
{
    ...
    if (dlg.exec() == QDialog::Accepted)
    {
        ...
        if (!Ipponboard::TournamentMode::WriteModes(configFile, dlg.Result(), errMsg))
        {
            ...
            QMessageBox::critical(
                this, QCoreApplication::tr("Error writing mode configurations"), errMsg);
...
void MainWindowTeam::on_comboBox_mode_currentIndexChanged(int i)
{
    ...
    {
        m_pUi->label_intermediate_result->setText(
            m_pUi->label_final_score->text()); //TODO: make this better!
...
{
    bool ok(false);
    const QString weights = QInputDialog::getText(this,
                                                  tr("Set Weights"),
...
void MainWindowTeam::on_actionSet_Round_Time_triggered()
{
    ...
    auto timeStr = QInputDialog::getText(this,
                                         tr("Set Value"),
...
void MainWindowTeam::paste_cell_content(QTableView* pTableView)
{
    ...
    {
        QMessageBox::warning(
            this, QApplication::applicationName(), tr("There is nothing to paste!"));
        return;
    }
    ...
    {
        QMessageBox::critical(
            this, QApplication::applicationName(), tr("Can not paste into an empty selection!"));
        return;
    }
